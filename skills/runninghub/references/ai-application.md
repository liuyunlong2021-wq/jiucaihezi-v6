...
If the workflow is already registered in `{baseDir}/data/my-workflows.json`, prefer
calling it by alias (`--workflow`) instead of hardcoding the webappId.
...
Before asking for a raw webappId, check whether the workflow is already registered:

```bash
python3 {baseDir}/scripts/runninghub_app.py --list-workflows
python3 {baseDir}/scripts/runninghub_app.py --workflow-info WORKFLOW_ALIAS
```
...
If the app is registered in `my-workflows.json`, use alias mode and friendly input names instead:

```bash
python3 {baseDir}/scripts/runninghub_app.py --workflow economy.shot.qwen_edit_3img \
  --node "prompt=图1 女子穿着图2 的衣服在图3 的咖啡厅窗边坐着" \
  --file "image1=/tmp/openclaw/rh-output/character.png" \
  --file "image2=/tmp/openclaw/rh-output/prop.png" \
  --file "image3=/tmp/openclaw/rh-output/scene.png" \
  -o /tmp/openclaw/rh-output/app_result_$(date +%s).png
```
...
# List registered workflow aliases
python3 {baseDir}/scripts/runninghub_app.py --list-workflows

# Inspect a registered workflow alias
python3 {baseDir}/scripts/runninghub_app.py --workflow-info economy.video.ltx23
...
Flags: `--node nodeId:fieldName=value`, `--file nodeId:fieldName=/path`, `--workflow WORKFLOW`, `--profile PROFILE`, `--instance-type default|plus`, `-o path`
Discovery: `--info WEBAPP_ID`, `--list-workflows`, `--workflow-info WORKFLOW`
